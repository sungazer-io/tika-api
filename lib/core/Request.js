...
export default class APIRequest {
  ...
  async _buildResponse(respObj) {
    ...
    if (contentType && contentType.includes('zip')) {
      //Unzip
      const contents = await JSZip.loadAsync(respObj.buffer());
      //Aggregate files
      let files = {};
      for (let fileKey of Object.keys(contents.files)) {
        const fileData = await contents.file(fileKey).async('string');
        if (fileKey == TikaDefinitions.MetadataFile) {
          files.metadata = new TikaMetadata(fileData);
        } else if (fileKey == TikaDefinitions.TextFile) {
          files.text = fileData;
        } else files[fileKey] = fileData;
      }
      //
